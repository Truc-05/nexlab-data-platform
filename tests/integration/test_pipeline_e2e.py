...
def write_sample_raw(spark, raw_zone: str):
    ...
    df.write.mode("overwrite").parquet(
        f"{raw_zone}/yellow_tripdata/year=2023/month=01/data.parquet"
    )
...
def write_sample_zones(spark, raw_zone: str):
    zones = spark.createDataFrame(
        [
            ...
            (100, "Brooklyn", "Downtown Brooklyn", "Boro Zone"),
            (200, "Bronx", "Hunts Point", "Boro Zone"),
        ],
        ["LocationID", "Borough", "Zone", "service_zone"],
    )
    zones.write.mode("overwrite").option("header", "true").csv(
        f"{raw_zone}/taxi_zones/taxi_zone_lookup.csv"
    )
