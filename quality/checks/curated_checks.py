...
def check_uniqueness(df: DataFrame, column: str, table: str):
    ...
    if duplicate_count > 0:
        raise DataQualityFailure(f"[{table}] column '{column}' has {duplicate_count} duplicate values")
...
def check_range(df: DataFrame, column: str, min_val: float, max_val: float, table: str):
    ...
    if out_of_range > 0:
        raise DataQualityFailure(f"[{table}] column '{column}' has {out_of_range} out-of-range values")
    logger.info("dq_pass_range", table=table, column=column, min=min_val, max=max_val)

def check_referential_integrity(fact: DataFrame, dim: DataFrame, fact_col: str, dim_col: str, table: str):
    ...
    if orphans > 0:
        logger.warning("dq_warn_referential_integrity", table=table, fk=fact_col, orphan_count=orphans)
    else:
...
def run(spark: SparkSession, year: int, month: int):
    ...
    check_referential_integrity(fact, dim_location, "pickup_location_sk", "location_sk", "fact_trips")
